...
from curiosity.stats import (
    MajorityLikes, TfidfFactBaseline,
    MajorityDialogActs, MajorityPolicyActs,
    save_metrics, fact_length_stats
)
...
def gen_configs(gpu, metrics_dir):
    ...
    all_configs = {
        ...
        # Ablations leaving on out
        'bert-known': {
            'disable_known_entities': 'true', 'use_glove': 'false', 'use_bert': 'true'},
        # Completely ablate out anything related to dialog acts
        'bert-da': {
            'disable_dialog_acts': 'true', 'use_glove': 'false', 'use_bert': 'true'},
        # Completely ablate out anything related to likes
        'bert-like': {
            'disable_likes': 'true', 'use_glove': 'false', 'use_bert': 'true'},
        # Completley ablate out anything related to facts
        'bert-facts': {
            'disable_facts': 'true', 'use_glove': 'false', 'use_bert': 'true'},
    }
    with open('run_allennlp.sh', 'w') as exp_f:
        ...
        for name, conf in all_configs.items():
            ...
            if job_gpu != -1:
                gpu_option = (
                    " -o '"
                    + json.dumps({'trainer': {'cuda_device': job_gpu}})
                    + "'"
                )
            else:
                gpu_option = ''
